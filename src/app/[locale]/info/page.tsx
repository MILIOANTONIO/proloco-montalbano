...
export default async function InfoPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      <h1 className="font-display text-3xl font-extrabold tracking-tight text-brand-900">{t.info.title}</h1>

      {grouped.length === 0 ? (
        <p className="text-brand-600">{t.info.empty}</p>
      ) : (
        grouped.map((g) => {
          ...
          return (
            <section key={g.cat}>
              <div className="mb-3 flex items-center gap-2.5">
                ...
                <h2 className="font-display text-lg font-bold text-brand-800">{g.label}</h2>
              </div>
              <ul className="space-y-3">
                {g.items.map((item) => (
                  <li key={item.id} className="rounded-3xl bg-white p-4 shadow-sm">
                    ...
                    {item.phone && (
                      <a href={`tel:${item.phone.replace(/\s/g, "")}`} className="mt-1 flex items-center gap-1.5 text-sm text-brand-600 hover:text-brand-800">
                    ...
                    {item.notes && <p className="mt-1 text-sm text-brand-500">{item.notes}</p>}
                  </li>
                ))}
              </ul>
            </section>
