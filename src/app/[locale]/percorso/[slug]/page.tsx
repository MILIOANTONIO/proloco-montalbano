    ...
    <article className="mx-auto max-w-2xl space-y-6">
      <div className="relative flex h-72 w-full items-center justify-center overflow-hidden rounded-3xl bg-gradient-to-br from-brand-100 to-olive-400/30 shadow-soft sm:h-96">
        {poi.coverImage ? (
          ...
          <Icon className="h-16 w-16 text-brand-500" />
        )}
        <Link
      ...
      <div className="space-y-3">
        <h1 className="font-display text-3xl font-bold text-brand-900">{tr.title}</h1>
        <div className="flex flex-wrap gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-100 px-3 py-1.5 text-xs font-medium text-brand-800">
      ...
      {tr.audioUrl && (
        <div className="rounded-2xl border border-brand-200 bg-white p-4">
          <p className="mb-2 text-sm font-medium text-brand-700">{t.poi.audio}</p>
      ...
      {tr.videoUrl && (
        <div className="rounded-2xl border border-brand-200 bg-white p-4">
          <p className="mb-2 text-sm font-medium text-brand-700">{t.poi.video}</p>
      ...
      {poi.chapters.map((chapter) => {
        ...
        return (
          <section key={chapter.id} className="space-y-3 border-t border-brand-200 pt-6">
            {chapter.imageUrl && (
              <div className="flex max-h-[420px] w-full items-center justify-center overflow-hidden rounded-2xl bg-brand-100">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={chapter.imageUrl} alt={ctr.heading} className="max-h-[420px] w-full object-contain" />
              </div>
            )}
            {ctr.heading && <h2 className="font-display text-2xl font-bold text-brand-900">{ctr.heading}</h2>}
